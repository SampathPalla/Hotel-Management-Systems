<%-- 
    Document   : PersonalBookingAnalysis
    Created on : Apr 12, 2015, 8:35:52 PM
    Author     : SampathYadav
--%>
...
<html>
    <head>
        ...
        <title>Personal Booking Data Analysis</title>
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                ...
                <%
                    ...
                    dataBasedOnSelection = stmt.executeQuery ("select r.location, COUNT(distinct g.guestid) AS Private_Guests from reservation r, guest g where r.guestid = g.guestid and g.TYPE ='I' and r.enddate between '01-JAN-"+yearSelected+"' and '31-DEC-"+yearSelected+"' group by location");
                    
                %>
                var data = new google.visualization.DataTable();
                data.addColumn('string', 'Location');
                data.addColumn('number', 'Private Guests');
                
                <%  while(dataBasedOnSelection.next()){ %>
                    data.addRow(["<%= dataBasedOnSelection.getString(1)%>",<%= dataBasedOnSelection.getString(2)%>]);
                <% } %>
                // Set chart options
                var options = {'title':"Personal Booking Guest Analysis for the Year" + "<%=yearSelected%>",
                               'width':500,
                               'height':500};
                var chart = null;
    ...
    <body>
        <div class="container" id="ownerHome">
            <ul class="nav nav-tabs nav-justified" id="tabAnalysis">
                <li class="active" id="liPersonal">
                    <a href="#tab-Personal" data-toggle="tab" onclick="drawChart()"><span class="glyphicon glyphicon-calendar"></span>   
                        <span>Personal Booking Analysis</span>
                    </a>
                </li>
            </ul>
