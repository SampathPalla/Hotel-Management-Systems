<%-- 
    Document   : HouseKeepingFeedbackAnalysis
    Created on : Apr 18, 2015, 6:07:49 PM
    Author     : Sahil
--%>
...
<html>
    <head>
        ...
        <title>Housekeeping Feedback Analysis</title>
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                ...
                <%
                    ...
                    dataBasedOnSelection = stmt.executeQuery ("select r.location, avg(f.fb_housekeeping) from reservation r, feedback f where f.reservationid = r.reservationid and r.enddate between '01-JAN-"+yearSelected+"' and '31-DEC-"+yearSelected+"' group by location");
                    
                %>
                var data = new google.visualization.DataTable();
                data.addColumn('string', 'Location');
                data.addColumn('number', 'Feedback : Housekeeping');
                
                <%  while(dataBasedOnSelection.next()){ %>
                    data.addRow(["<%= dataBasedOnSelection.getString(1)%>",<%= dataBasedOnSelection.getString(2)%>]);
                <% } %>
                // Set chart options
                var options = {'title': "Housekeeping Feedback Analysis for the year " + "<%=yearSelected%>",
                               'width':500,
                               'height':500};
                var chart = null;
        ...
        <div class="container" id="ownerHome">
            <ul class="nav nav-tabs nav-justified" id="tabAnalysis">
                <li class="active" id="liDomesticGuest">
                    <a href="#tab-HousekeepingFeedbackAnalysis" data-toggle="tab" onclick="drawChart()"><span class="glyphicon glyphicon-calendar"></span>   
