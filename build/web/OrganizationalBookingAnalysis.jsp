<%-- 
    Document   : OrganizationalBookingAnalysis
    Created on : Apr 12, 2015, 8:36:25 PM
    Author     : SampathYadav
--%>
...
<html>
    <head>
        ...
        <title>Organizational Booking Data Analysis</title>
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                ...
                <%
                    ...
                    dataBasedOnSelection = stmt.executeQuery ("select r.location, COUNT(distinct g.guestid) AS Company_Guests from reservation r, guest g where r.guestid = g.guestid and g.TYPE ='C' and r.enddate between '01-JAN-"+yearSelected+"' and '31-DEC-"+yearSelected+"' group by location");
                    
                %>
                var data = new google.visualization.DataTable();
                data.addColumn('string', 'Location');
                data.addColumn('number', 'Company Guests');
                
                <%  while(dataBasedOnSelection.next()){ %>
                    data.addRow(["<%= dataBasedOnSelection.getString(1)%>",<%= dataBasedOnSelection.getString(2)%>]);
                <% } %>
                // Set chart options
                var options = {'title':"Organizational Booking Guest Analysis for the Year" + "<%=yearSelected%>",
                               'width':500,
                               'height':500};
                var chart = null;
    ...
    <body>
        <div class="container" id="ownerHome">
            <ul class="nav nav-tabs nav-justified" id="tabAnalysis">
                <li class="active" id="liOrganizational">
                    <a href="#tab-Organzational" data-toggle="tab" onclick="drawChart()"><span class="glyphicon glyphicon-calendar"></span>   
                        <span>Organizational Booking Analysis</span>
                    </a>
                </li>
            </ul>
